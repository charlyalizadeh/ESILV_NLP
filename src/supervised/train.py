...
@click.pass_context
@click.argument('trainpath')
@click.argument('modelname')
@click.argument('out', type=click.File('w'), required=False)
@click.argument('testpath', required=False)
def train_supervised(ctx, trainpath, modelname, out, testpath):
    ...
    if modelname == 'ffnn':
        d['X_val'] = X_val
        d['y_val'] = y_val
        d['loss_fig_out'] = f'./report/images/loss_{basename(trainpath)[6:-4]}.pgf'
    ...
    if not out:
        click.echo(f'{modelname} on {trainpath} \n\
...
Confusion matrix:\n {val_cm}\n',
    ...
    #print(X_test)
    #y_test_pred = predict_func[modelname](X_test, model)
    #y_test_pred.savetext('result_test.txt')
